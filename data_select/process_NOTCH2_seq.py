'''
This program produces BED files and bash script. The bash script is used to extract contig with NOTCH2 and NOTCH2NLR.
...
<NOTCH2_R_anno> - subset of NOTCH2 and NOTCH2NLR genes annotations from all the HPRC assembly annotations (.gff3)

Input:
List of assemblies to extract NOTCH2 and NOTCH2NLR sequences from. e.g. HG00438#1
Output:
<prefix>.bed - BED files subsetting each NOTCH2 sequence from contig. Contains NOTCH2 and NOTCH2NLR
get_N2_seq_from_assembly.sh - a bash script using samtools faidx to extract contigs from original assemblies
'''
...
with open('NOTCH2_R_anno', "r") as myfile:
    for line in myfile.readlines():
        anno.append(line.rstrip('-2022_07-genes.gff3\n').lstrip('Homo_sapiens-GCA_').split(' '))
...
contig_frame[2] = np.where(contig_frame[1] == 'matern', sample_id.loc[contig_frame.index][1], sample_id.loc[contig_frame.index][2])
contig_frame[2] = contig_frame[2].str.replace(r'\..*', '', regex=True)
contig_frame[2] = contig_frame[2].str.replace('GCA_', '', regex=True)
...
contig_frame = contig_frame.set_index(2)
...
### Output bash script
with open("get_N2_seq_from_assembly.sh", "w") as f:
    # Write header
    f.write('#!/bin/bash\n')
    f.write('\n')
    f.write('set -e\n')
...
#### Bed files to extract the first 4 exon region of NOTCH2
# for index, row in contig_frame.iterrows():
#     indexAnno = annotation[annotation[0] == index].index
#     indexAnno_i = annotation.index.get_loc(indexAnno[0])
#     print(index)
...
#     # print(indexAnno)
#     # print(indexAnno_i)
#     print(annotation.iloc[indexAnno_i+1, 0])
#     with open(str(annotation.iloc[indexAnno_i+1, 0] + "_NL2_exon5reg.bed"), "w") as f:
#         f.write(str(annotation.iloc[indexAnno_i + 1, 3]) + '::' + str(annotation.iloc[indexAnno_i + 1, 0]) + ':' + str(annotation.iloc[indexAnno_i + 1, 1]) + '-' + str(annotation.iloc[indexAnno_i + 1, 2]) + '(' + str(annotation.iloc[indexAnno_i + 1, 5]) + ')' + '\t' + '0' + '\t' + '83500' + '\t' + str(annotation.iloc[indexAnno_i + 1, 3]) + '\t' + str(annotation.iloc[indexAnno_i + 1, 4]) + '\t' + str(annotation.iloc[indexAnno_i + 1, 5]) + '\n')
